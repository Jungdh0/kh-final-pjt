...
const Result = ({ resultVisible, visible, hidden, select }) => {
  ...
  return (
    <>
      <section
        id="result"
        class="mx-auto mt-5 py-5 px-3"
        style={resultVisible ? visible : hidden}
      >
        ...
        <div class="px-3 mt-3 text-center">
          ...
          <div class="resultDesc col-lg-6 col-md-8 col-sm-10 mx-auto py-3">
            {infoList[point].desc}
          </div>
        </div>
        <div class="row" id="resultImgs">
          <div class="col-sm-4">
            <img src={leftURL} class="leftImage" alt={point} width="100%" />
          </div>
          <div class="col-sm-4">
            <img src={centerURL} class="centerImage" alt={point} width="100%" />
          </div>
          <div class="col-sm-4">
            <img src={rightURL} class="rightImage" alt={point} width="100%" />
          </div>
        </div>
        <div>
          <button class="default-btn mt-3">더 많은 추천 보러가기</button>
        </div>
        <div>
          <button type="button" class="btn btn-warning mt-3 mx-auto">
            {/* <img
